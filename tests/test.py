"""
Deep SAD - 오분류 샘플 + TP 샘플 시각화 (MNIST)
- deepsad_best.pt 로드
...
def get_test_loader(normal_class, batch_size, num_workers, img_size):
    ...
    raw_imgs = mnist_test.data.numpy()
...
def draw_result_image(
    ...
    img = cv2.resize(raw_img, (canvas_size, canvas_size), interpolation=cv2.INTER_NEAREST)
    img = cv2.cvtColor(img, cv2.COLOR_GRAY2BGR)

    t = 6
    cv2.rectangle(img, (t // 2, t // 2), (canvas_size - t // 2, canvas_size - t // 2), border_color, t)
    ...
    cv2.putText(img, f"idx={idx}", (canvas_size - 75, canvas_size - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (160, 160, 160), 1, cv2.LINE_AA)
...
def save_and_display(
    ...
    for i, idx in enumerate(display_idx):
        ...
        save_path = os.path.join(
            result_dir, sample_type,
            f"{sample_type}_{i:04d}_idx{idx}_score{scores[idx]:.4f}.png",
        )
        cv2.imwrite(save_path, img)
        img_list.append(img)

    print(f"  {sample_type} 이미지 저장: {result_dir}/{sample_type}/ ({len(display_idx)}개)")
    ...
    rows = [np.hstack(img_list[r * grid_cols:(r + 1) * grid_cols]) for r in range(n_rows)]
    grid = np.vstack(rows)
    ...
    cv2.putText(
        ...
        (10, 36), cv2.FONT_HERSHEY_SIMPLEX, 0.78, label_color, 2, cv2.LINE_AA,
    )
...
def main():
    ...
    print("\n[4단계] 샘플 탐지")
    fp_idx, fn_idx, tp_idx = find_samples(scores, test_labels, threshold, rng, CFG["num_tp_samples"])
    ...
    for idx_arr, stype in [(fp_idx, "FP"), (fn_idx, "FN"), (tp_idx, "TP")]:
        save_and_display(
            raw_imgs=raw_imgs, indices=idx_arr, scores=scores, labels=test_labels,
            threshold=threshold, sample_type=stype, result_dir=CFG["result_dir"],
        )
