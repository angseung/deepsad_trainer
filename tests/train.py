...
def main():
    ...
    for epoch in epoch_pbar:
        train_loss = train_one_epoch(
            model, train_loader, c, optimizer, device,
            epoch, CFG["n_epochs"], CFG["eta"],
        )
        scheduler.step()
        ...
        if epoch % 10 == 0 or epoch == CFG["n_epochs"]:
            ...
            if auc > best_auc:
                ...
                best_epoch = epoch
                save_checkpoint(
                    path=CFG["save_path"], epoch=epoch, model=model,
                    optimizer=optimizer, scheduler=scheduler,
                    c=c, best_auc=best_auc, cfg=CFG,
                )
                tqdm.write(
                    f"  ✓ Best 모델 저장 | Epoch {epoch:>3} | AUC {best_auc:.4f}"
                )
